...
TUI_ACTION_NOTES = {
    ...
    "delete_migrated_originals": [
        ...
        "进入列表后可预览、勾选，按 a 选中全部匹配项，再按 x 二次确认删除。",
    ],
    "delete_archived_sessions": [
        "进入归档会话列表，先预览再删除。",
        "支持空格逐条勾选，也支持按 a 选中全部匹配项后按 x 删除。",
        "会同步移除 Desktop 线程栏和本工具索引里的归档记录。",
    ],
    "list_sessions": [
        "内置会话浏览器，支持搜索、预览、详情查看和导出；默认读取 Codex 会话。",
        "可按 g 切换 Agent：Codex、Claude Code、Pi、ZCode 或全部 Agent 汇总；非 Codex 会话导出为文件级 Bundle。",
        "支持空格逐条勾选，也支持按 a 选中全部匹配项后按 e 导出。",
    ],
    "project_sessions": [
    ...
    "list_skills": [
        ...
        "支持当前 Skill、勾选多条导出，也支持按 a 选中全部匹配自定义 Skills 后按 e 导出。",
    ],
    ...
    "delete_skill": [
        ...
        "支持空格逐条勾选，也支持按 a 选中全部匹配自定义 Skills 后按 x 删除。",
    ],
    "github_status": [
...
def build_tui_menu_actions() -> List[TuiMenuAction]:
    return [
        ...
        _menu_action("github_proxy", "n", "连接/断开代理", ("github-proxy", "<proxy_url>")),
