...
def collect_bundle_summaries(
    ...
    *,
    ...
    for bundle_dir in iter_bundle_directories_under_root(bundle_root):
        ...
        skills_manifest_path = bundle_dir / "skills_manifest.json"
        has_skills_manifest = skills_manifest_path.is_file()
        bundled_skill_count = 0
        used_skill_count = 0
        if has_skills_manifest:
            try:
                skills_data = json.loads(skills_manifest_path.read_text(encoding="utf-8"))
                bundled_skill_count = skills_data.get("bundled_skill_count", 0)
                used_skill_count = skills_data.get("used_skill_count", 0)
            except Exception:
                pass
